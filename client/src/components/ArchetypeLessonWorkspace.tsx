...
export function ArchetypeLessonWorkspace({ lesson, accent, onBack }: { lesson: LessonDefinition; accent: Accent; onBack: () => void }) {
  ...
  const renderAuthoredActivity = (activity: LessonActivity) => <div key={activity.id} className="rounded-xl border border-current/15 bg-white/60 p-4">
    ...
    {activity.speakingLines?.length ? <div className="mt-4 space-y-3">{activity.kind === "listening" && !revealedTranscripts[activity.id] ? <div className="rounded-lg border border-[#d9e5da] bg-[#f7fbf7] p-3"><p className="text-sm font-semibold text-[#253453]">Listen first. The transcript is hidden so you can notice the message before reading it.</p><p dir="rtl" className="arabic mt-2 text-right text-sm text-[#526078]">استمع أولاً. النص مخفي حتى تلاحظ الرسالة قبل قراءتها.</p><button type="button" onClick={() => setRevealedTranscripts((current) => ({ ...current, [activity.id]: true }))} className="mt-3 rounded-lg border border-[#c5d9ca] bg-white px-3 py-2 text-sm font-bold text-[#316647]">Reveal transcript / أظهر النص</button></div> : null}{activity.speakingLines.map((line) => <div key={line.id} className="flex items-start justify-between gap-4 rounded-lg border border-[#d9e5da] bg-white p-3"><div>{activity.kind === "listening" && !revealedTranscripts[activity.id] ? <p className="font-semibold text-[#253453]">Listen to this clip, then decide what was reported and who accepted responsibility.</p> : <><p className="font-semibold text-[#253453]">{line.text}</p><p dir="rtl" className="arabic mt-1 text-right text-sm text-[#526078]">{line.textArabic}</p></>}<p className="mt-2 text-xs text-[#63718a]">{line.pronunciationHint}</p></div><button type="button" aria-label={`Play ${activity.kind === "listening" ? "listening clip" : line.text}`} onClick={() => speak(line.audioText ?? line.text, accent)} className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-[#c5d9ca] bg-[#f7fbf7] text-[#316647]"><Volume2 className="h-4 w-4" /></button></div>)}</div> : null}
    ...
    {activity.retrievalCheck ? <div className="mt-4 rounded-lg border border-[#d9e5da] bg-[#f7fbf7] p-3"><p className="text-xs font-bold uppercase tracking-[.12em] text-[#397558]">Quick check · تحقق سريع</p><p className="mt-2 text-sm font-semibold leading-6 text-[#253453]">{activity.retrievalCheck.prompt}</p><p dir="rtl" className="arabic mt-1 text-right text-sm leading-6 text-[#526078]">{activity.retrievalCheck.promptArabic}</p><details className="mt-3 text-xs text-[#63718a]"><summary className="cursor-pointer font-semibold">What to show · ماذا تُظهر؟</summary><p className="mt-2">{activity.retrievalCheck.expectedEvidence}</p><p dir="rtl" className="arabic mt-1 text-right">{activity.retrievalCheck.expectedEvidenceArabic}</p></details></div> : null}
  </div>;
