...
import { A1_LESSONS, A2_LESSONS, B1_LESSONS, B2_LESSONS, C1_LESSONS, C2_LESSONS, type LessonDefinition } from "@shared/course";
...
const levels = [
  ...
  { level: "C1", label: "Advanced", arabic: "متقدم", detail: "20 lessons · nuance & evidence" },
  { level: "C2", label: "Mastery", arabic: "إتقان", detail: "16 lessons · independent judgement" },
] as const;
...
const tutorialCopy: Record<ActiveLevel, { title: string; titleArabic: string; intro: string; introArabic: string; steps: Array<{ title: string; arabic: string; body: string }> }> = {
  A1: { title: "Your first English route", titleArabic: "طريقك الأول في الإنجليزية", intro: "English Journey keeps the next step visible. You will meet useful words, notice a grammar pattern, practise it, and finish with a contextual quiz.", introArabic: "تُبقي رحلة الإنجليزية خطوتك التالية واضحة. ستتعرّف على كلمات مفيدة، وتلاحظ قاعدة، وتتدرّب عليها، ثم تنهي باختبار سياقي.", steps: [{ title: "Follow the seven steps", arabic: "اتبع الخطوات السبع", body: "Start together, make language familiar, give it shape, try it, read, write, and check your progress." }, { title: "Use sound and Arabic wisely", arabic: "استخدم الصوت والعربية بذكاء", body: "Play examples aloud, read the Arabic support when you need it, then return your attention to the English." }, { title: "A pass unlocks the next lesson", arabic: "النجاح يفتح الدرس التالي", body: "The lesson quiz checks meaning and grammar in context. You need 80% to continue, and you can review before trying again." }] },
  A2: { title: "Build independence", titleArabic: "ابنِ استقلالك", intro: "At A2, the mentor connects everyday language to your own life so each lesson becomes something you can use.", introArabic: "في A2، يربط المرشد اللغة اليومية بحياتك حتى يصبح كل درس شيئاً يمكنك استخدامه.", steps: [{ title: "Read the mentor first", arabic: "اقرأ كلام المرشد أولاً", body: "The opening explains why the lesson matters and tells you what you will be able to do." }, { title: "Move in order", arabic: "تقدّم بالترتيب", body: "Vocabulary leads to grammar, practice leads to reading and writing, and the final quiz checks the whole route." }, { title: "Use prompts for extra help", arabic: "استخدم الطلبات للمساعدة", body: "Copy a lesson-aware prompt into the external AI tool you choose when you want another explanation or feedback." }] },
...
function FirstUseTutorial({ level, open, onOpenChange }: { level: ActiveLevel; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto border-[#e5ddcf] bg-[#fffdf7] text-[#253453]"><DialogHeader><DialogTitle className="text-2xl font-bold">{copy.title}</DialogTitle><DialogDescription className="leading-7 text-[#627087]">{copy.intro}</DialogDescription><p dir="rtl" className="arabic text-right text-sm leading-7 text-[#718098]">{copy.titleArabic} · {copy.introArabic}</p></DialogHeader><div className="mt-2 space-y-3">{copy.steps.map((step, index) => <div key={step.title} className="rounded-2xl border border-[#e5ddcf] bg-white p-4"><div className="flex items-start gap-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-[#fff0bd] text-sm font-black text-[#765618]">{index + 1}</span><div><h3 className="font-bold">{step.title}</h3><p dir="rtl" className="arabic mt-1 text-right text-xs text-[#8390a4]">{step.arabic}</p><p className="mt-2 text-sm leading-7 text-[#5c6b81]">{step.body}</p></div></div></div>)}</div><Button onClick={() => onOpenChange(false)} className="mt-2 rounded-xl bg-[#253453] text-white hover:bg-[#35476d]">Start this route</Button></DialogContent></Dialog>;
}
...
export function AppShell({ initialSearch }: { initialSearch?: string } = {}) {
  ...
  const [view, setView] = useState<ShellView>(() => new URLSearchParams(routeSearch).get("review") === "1" ? "review" : "course");
  ...
  const lessons = activeLevel === "C2" ? C2_LESSONS : activeLevel === "C1" ? C1_LESSONS : activeLevel === "B2" ? B2_LESSONS : activeLevel === "B1" ? B1_LESSONS : activeLevel === "A2" ? A2_LESSONS : A1_LESSONS;
  ...
  useEffect(() => {
    try {
      const key = `english-journey:tutorial:${activeLevel}:seen`;
      setTutorialOpen(window.localStorage.getItem(key) !== "1");
    } catch { setTutorialOpen(false); }
  }, [activeLevel]);
  const closeTutorial = (open: boolean) => {
    setTutorialOpen(open);
    if (!open) {
      try { window.localStorage.setItem(`english-journey:tutorial:${activeLevel}:seen`, "1"); } catch {}
    }
  };
  useEffect(() => {
...
export function CourseDashboard({ level, lessons, completedLessons, profile, openLesson, canEnter, unlocked }: { level: ActiveLevel; lessons: LessonDefinition[]; completedLessons: Set<number>; profile?: { totalXp: number; currentStreak: number; longestStreak: number } | null; openLesson: (lesson: LessonDefinition) => void; canEnter: (number: number) => boolean; unlocked: boolean }) {
