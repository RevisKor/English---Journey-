"""Replace repeated B1 target items with fresh, lesson-specific equivalents."""
...
def main() -> None:
    ...
    for lesson in lessons:
        for index, item in enumerate(lesson["vocabulary"]):
            ...
            if key in seen:
                ...
                seen.add(key)
    if not duplicates:
        print("No repeated B1 targets found.")
        return
    ...
    for lesson_number, indices in duplicates.items():
        ...
        prompt = f"""You are repairing original B1 course vocabulary. Replace only the repeated target entries below with fresh, useful B1 terms or collocations that fit this lesson. Do not use any already-used target. Give Modern Standard Arabic support and original examples. Keep each item concise.
...
Return exactly one replacement per requested index in the required JSON."""
        ...
        for attempt in range(3):
            response = client.chat.completions.create(model=MODEL, messages=[{"role": "system", "content": "You are a precise British English curriculum editor. Output only the requested JSON."}, {"role": "user", "content": prompt + f"\nThis is validation retry {attempt + 1}; every replacement must be different from the avoid list and from the other replacements."}], max_completion_tokens=1800, response_format={"type": "json_schema", "json_schema": {"name": "b1_replacements", "strict": True, "schema": ITEM_SCHEMA}})
            ...
            if sorted(item.get("index") for item in candidate) == indices and len(candidate_keys) == len(set(candidate_keys)) and all(key not in seen for key in candidate_keys):
                ...
                break
        if not replacements:
            raise ValueError(f"Lesson {lesson_number} could not produce unique replacement entries after validation retries")
        ...
        print(f"Repaired lesson {lesson_number}: {len(indices)} repeated targets")
