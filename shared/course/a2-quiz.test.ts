...
describe("A2 assessment definitions", () => {
  it("creates a balanced eight-question lesson quiz for every A2 lesson", () => {
    for (let lessonNumber = 1; lessonNumber <= 135; lessonNumber += 1) {
  ...
  it("creates a thirty-question module test and three reading checks", () => {
    for (let moduleNumber = 1; moduleNumber <= 9; moduleNumber += 1) {
      expect(buildA2ModuleTest(moduleNumber)).toHaveLength(30);
    }
