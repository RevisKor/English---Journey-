...
describe("milestone assessment routing", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    mocks.getLearnerProgress.mockResolvedValue({ lessons: [{ lessonNumber: 14, status: "completed" }], modules: [] });
    ...
    assessmentMocks.gradeAssessmentInstance.mockResolvedValue({ level: "C2", lessonNumber: 15, moduleNumber: 1, assessmentType: "milestone_quiz", score: 92, missedItemKeys: [] });
  });

  it("creates a distinct milestone instance for the C2 checkpoint", async () => {
    const caller = courseRouter.createCaller(createContext());
    const assessment = await caller.milestoneQuiz({ level: "C2", lessonNumber: 15 });
  ...
  it("submits a milestone checkpoint without converting it into a cumulative module test", async () => {
    const caller = courseRouter.createCaller(createContext());
    await caller.submitMilestoneQuiz({ level: "C2", lessonNumber: 15, assessmentInstanceId: 207, answers: { "milestone-q": "answer" } });
    expect(assessmentMocks.gradeAssessmentInstance).toHaveBeenCalledWith(expect.objectContaining({
      scope: { level: "C2", assessmentType: "milestone_quiz", lessonNumber: 15, moduleNumber: 1 },
    }));
    expect(mocks.submitLessonAssessment).toHaveBeenCalledWith(expect.objectContaining({
      level: "C2", assessmentType: "milestone_quiz", lessonNumber: 15, moduleNumber: 1,
    }));
  });
});
